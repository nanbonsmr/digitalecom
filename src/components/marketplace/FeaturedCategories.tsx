...
const FeaturedCategories = () => {
    ...
    const fetchCategoryCounts = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from("products").select("category").eq("is_published", true);
  ...
  return <section className="py-10 md:py-16 lg:py-20 bg-secondary/30">
      <div className="container px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
          {categoryConfigs.map(category => <Card key={category.label} className="group cursor-pointer overflow-hidden border-border/50 hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:-translate-y-1" onClick={() => handleCategoryClick(category.dbCategory)}>
              ...
              <CardContent className="absolute bottom-0 left-0 right-0 p-2 md:p-3 text-white">
                ...
                {isLoading ? <Skeleton className="h-3 w-12 md:w-16 bg-white/20" /> : <p className="text-[10px] md:text-xs text-white/80">
